...
if (( $(echo "$cpuUsage > $cpuThreshold" | bc -l) )); then
    echo "CPU usage exceeded $cpuThreshold%" | mailx -s "CPU Usage Alert" pratyushpritam88@gmail.com
fi
if (( $(echo "$memoryUsage > $memoryThreshold" | bc -l) )); then
    echo "Memory usage exceeded $memoryThreshold%" | mailx -s "Memory Usage Alert" pratyushpritam88@gmail.com
fi
if (( $(echo "$diskSpaceUsage > $diskSpaceThreshold" | bc -l) )); then
    echo "Disk space usage exceeded $diskSpaceThreshold%" | mailx -s "Disk Space Alert" pratyushpritam88@gmail.com
fi
if (( activeProcesses > activeProcessesThreshold )); then
    echo "Active processes exceeded $activeProcessesThreshold" | mailx -s "Active Processes Alert" pratyushpritam88@gmail.com
fi
